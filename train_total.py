    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_index, (src_data, tgt_data, test_data, pse_data) in enumerate(zip(src_loader, tgt_loader, test_loader, pse_loader)):
            ...
            if (batch_index+1) % printfrq == 0:
                print('epoch %d/%d:  %d/%d time: %.2f srcmiu = %.1f tgtmiu = %.1f src_loss = %.3f tgt_loss = %.3f con_loss = %.3f pse_loss = %.3f\n'%(epoch+1,args.num_epoch, batch_index+1,num_batches, batch_time, np.mean(loss_hist[index_i+1-printfrq:index_i+1,5])*100, np.mean(loss_hist[index_i+1-printfrq:index_i+1,6])*100, np.mean(loss_hist[index_i+1-printfrq:index_i+1,1]), np.mean(loss_hist[index_i+1-printfrq:index_i+1,2]), np.mean(loss_hist[index_i+1-printfrq:index_i+1,3]), np.mean(loss_hist[index_i+1-printfrq:index_i+1,4]) ))
